-- Fix cloud deep-merge ghosts for delete-semantic maps (execution / expedition).
--
-- Symptom: after completing one-shot expeditions (e.g. swamp sanctuary / black tree,
-- occultist chamber), every reload re-opens those dialogs and refunds the locked
-- expedition villagers again (~150+ per reload for late-game saves).
--
-- Cause: `jsonb_deep_merge_objects` only walks keys present in the patch. Sending
-- `executionStartTimes: {}` / `expeditionVillagers: {}` after a completion does NOT
-- remove stale keys from the existing cloud object — completed actions stay "in
-- flight" forever. On load, `flushOverdueActionExecutions` re-completes them
-- (bypassing `shouldShowAction`) and adds expedition villagers back to free.
--
-- Fix: after deep-merge on incremental saves, wholesale-replace any delete-semantic
-- top-level maps present in the incoming diff with the patch value (same semantics
-- as restart's `existing || diff` for those keys only). Client already always sends
-- full objects for these slices (see client/src/game/save.ts).
--
-- Client hardening (same bug): reconcileInFlightExecutionsOnLoad drops completed
-- one-shot ghosts without refunding; completeActionExecution no-ops re-runs.
...
BEGIN
  ...
  IF v_existing_state IS NOT NULL THEN
    ...
    ELSE
      ...
      -- ========== DELETE-SEMANTIC MAP WHOLESALE REPLACE ==========
      -- These maps remove keys when actions complete. Deep-merge cannot express
      -- deletions, so when the client sends the full object, replace it wholesale.
      -- Only maps the client always sends as full objects (see save.ts). Do NOT
      -- include cooldowns here — those arrive as partial diffs and must deep-merge.
      FOREACH v_slice IN ARRAY ARRAY[
        'executionStartTimes',
        'executionDurations',
        ...
        IF p_game_state_diff ? v_slice THEN
          v_merged_state := jsonb_set(
            v_merged_state,
            ARRAY[v_slice],
            COALESCE(p_game_state_diff->v_slice, '{}'::jsonb),
            true
          );
        END IF;
      END LOOP;
      -- ========== END DELETE-SEMANTIC MAP WHOLESALE REPLACE ==========
    END IF;
  ELSE
...
COMMENT ON FUNCTION save_game_with_analytics(JSONB, JSONB, JSONB, BOOLEAN, BOOLEAN) IS
  'Incremental cloud save with deep-merge, wholesale replace of delete-semantic maps (execution/expedition), and permanent-item protection (tools/weapons/books).';
