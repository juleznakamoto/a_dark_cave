-- Protect server-written referrals from full-document cloud save wipes.
-- processReferral updates the referrer's game_state.referrals directly; a stale
-- client full-replace was overwriting that array with [] and permanently losing
-- invite credit (new user already had referralProcessed=true).
-- Body is save_game_with_analytics from migration 036 plus:
--   1) SELECT ... FOR UPDATE on game_saves
--   2) union-merge of referrals by userId after replace/merge
...
COMMENT ON FUNCTION public.merge_game_state_referrals(JSONB, JSONB) IS
  'Union-merge referrals by userId (claimed OR, timestamp max). Used by save_game_with_analytics so full-replace cannot wipe server-written invite rewards.';

REVOKE ALL ON FUNCTION public.merge_game_state_referrals(JSONB, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.merge_game_state_referrals(JSONB, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.merge_game_state_referrals(JSONB, JSONB) TO service_role;
...
BEGIN
  ...
  -- Lock the row for the duration of this transaction so processReferral and
  -- autosave cannot lose each other's writes (read-modify-write race).
  ...
  -- ========== REFERRAL PROTECTION ==========
  -- Always union-merge (including restart): invite rewards are account-level and
  -- must survive stale full-document client payloads.
  IF v_existing_state IS NOT NULL THEN
    v_merged_state := public.merge_game_state_referrals(v_existing_state, v_merged_state);
  END IF;
  -- ========== END REFERRAL PROTECTION ==========
  ...
  THEN
    ...
    BEGIN
      ...
      IF v_silver_delta > 10000 AND NOT v_achievements_changed THEN
        RAISE EXCEPTION 'Save rejected: silver delta % exceeds max 10000 per save (claimedAchievements unchanged)',
          v_silver_delta;
      END IF;
      ...
      IF v_gold_delta > 5000 AND NOT v_purchases_changed THEN
        RAISE EXCEPTION 'Save rejected: gold delta % exceeds max 5000 per save (activatedPurchases unchanged)',
          v_gold_delta;
      END IF;
    END;
  END IF;
...
COMMENT ON FUNCTION public.save_game_with_analytics(JSONB, JSONB, JSONB, BOOLEAN, BOOLEAN, BOOLEAN) IS
  'Cloud save: deep-merge by default; p_full_replace=true stores the payload as the full game_state. Permanent tools/weapons/books protection and referral union-merge still apply (referrals also on restart).';
