-- Guard against a class of save corruption that wiped players' `tools` object.
--
-- Symptom: a player logs in and the Mine tab + tool-crafting buttons vanish because
-- every key in `game_state.tools` is `false`, even though `story.seen.*` craft flags and
-- their resources/weapons prove they owned the full pickaxe/axe/lantern chain.
--
-- Cause: an incremental cloud-save diff can arrive with the `tools` object all-`false`
-- (client edge case: a load/cross-device diff producing an empty/partial `tools`). Both the
