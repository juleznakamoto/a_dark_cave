-- Guard against hybrid save corruption that wipes `buildings` (and sticky unlock flags)
-- while leaving tools/story intact — sibling of migration 025's tools wipe.
--
-- Symptom: high playTime + rich story.seen / owned tools, but every buildings.* count is 0
-- (village gone; Explore Temple still visible via obsidian_lantern). Same root class as tools:
-- client load leaves createInitialState() zeros for buildings, then an incremental deep-merge
-- save writes the full all-zero buildings object and zeros every key.
--
-- Fix (incremental saves only; restarts with p_allow_playtime_overwrite exempt):
--   1) Per building key: keep GREATEST(merged, existing), except woodenHut which may drop by
--      at most 1 (madness / fireStorm / village-attack outcomes).
--   2) Sticky true flags (gameStarted, villageUnlocked, forestUnlocked, …) cannot go true→false.
--
-- Tools/weapons/books protection from 025 is retained below.
...
DECLARE
  ...
  v_owned_items JSONB;
  v_building_key TEXT;
  v_old_building NUMERIC;
  v_new_building NUMERIC;
  v_protected_buildings JSONB;
  v_flag_key TEXT;
BEGIN
  ...
  IF v_existing_state IS NOT NULL THEN
    IF p_allow_playtime_overwrite THEN
      -- Full reset / restart: each top-level key in the diff replaces wholesale.
  ...
  IF v_existing_state IS NOT NULL AND NOT p_allow_playtime_overwrite THEN
    ...
    -- Buildings: never wipe counts via a hybrid all-zero patch. woodenHut may drop by 1.
    IF v_existing_state ? 'buildings'
       AND jsonb_typeof(v_existing_state->'buildings') = 'object'
    THEN
      v_protected_buildings := COALESCE(v_merged_state->'buildings', '{}'::jsonb);
      FOR v_building_key IN
        SELECT jsonb_object_keys(v_existing_state->'buildings')
      LOOP
        v_old_building := COALESCE(
          (v_existing_state->'buildings'->>v_building_key)::NUMERIC,
          0
        );
        v_new_building := COALESCE(
          (v_protected_buildings->>v_building_key)::NUMERIC,
          0
        );

        IF v_building_key = 'woodenHut' THEN
          -- Allow at most -1 per save (event damage); block wipe-to-zero from mid-game.
          IF v_new_building < v_old_building - 1 THEN
            v_new_building := v_old_building;
          END IF;
        ELSIF v_new_building < v_old_building THEN
          v_new_building := v_old_building;
        END IF;

        v_protected_buildings := jsonb_set(
          v_protected_buildings,
          ARRAY[v_building_key],
          to_jsonb(v_new_building),
          true
        );
      END LOOP;
    ...
    -- Sticky unlock / progression flags: once true, incremental saves cannot clear them.
    IF v_existing_state ? 'flags'
       AND jsonb_typeof(v_existing_state->'flags') = 'object'
    THEN
      FOREACH v_flag_key IN ARRAY ARRAY[
        ...
        IF (v_existing_state->'flags'->>v_flag_key) = 'true' THEN
          v_merged_state := jsonb_set(
            v_merged_state,
            ARRAY['flags', v_flag_key],
            'true'::jsonb,
            true
          );
        END IF;
    ...
    END IF;
  END IF;
  -- ========== END PERMANENT-ITEM / BUILDINGS / FLAGS PROTECTION ==========
  ...
  THEN
    ...
    BEGIN
      ...
      IF v_silver_delta > 5000 AND NOT (p_game_state_diff ? 'claimedAchievements') THEN
        RAISE EXCEPTION 'Save rejected: silver delta % exceeds max 5000 per save (claimedAchievements not in diff)',
          v_silver_delta;
      END IF;

      -- Validate gold delta: max +2000 per save, unless activatedPurchases changed
      ...
      IF v_gold_delta > 2000 AND NOT (p_game_state_diff ? 'activatedPurchases') THEN
        RAISE EXCEPTION 'Save rejected: gold delta % exceeds max 2000 per save (activatedPurchases not in diff)',
          v_gold_delta;
      END IF;
    END;
  END IF;
...
COMMENT ON FUNCTION save_game_with_analytics(JSONB, JSONB, JSONB, BOOLEAN, BOOLEAN) IS
  'Incremental cloud save with deep-merge + permanent-item protection (tools/weapons/books add-only) + buildings floor (woodenHut -1 max) + sticky unlock flags.';
...
-- only the 5-arg (p_game_state_diff, ...) signature above.
DROP FUNCTION IF EXISTS save_game_with_analytics(UUID, JSONB, JSONB, BOOLEAN);
DROP FUNCTION IF EXISTS save_game_with_analytics(UUID, JSONB, JSONB, BOOLEAN, BOOLEAN);
