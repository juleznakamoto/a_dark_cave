-- Exclude sessions at the 24h ping duration cap from intra-day volume.
-- Start is estimated as updated_at - duration_seconds; capped rows all land
-- near "now - 24h" and inflate the leftmost bucket of the 24h chart.
...
AS $$
  ...
  sessions AS (
    ...
    WHERE sv.duration_seconds < 86400
      AND sv.updated_at >= w.end_bucket - w.lookback - INTERVAL '1 day'
...
COMMENT ON FUNCTION get_session_intraday_stats(INTEGER, INTEGER) IS
  'Admin intra-day session counts; start ≈ updated_at - duration_seconds; excludes 24h-capped rows';
