...
export const gameStateSchema = z.object({
  ...
  resources: z
    .object({
      ...
      veinroot: z.number().min(0).default(0),
      ...
      veinfire_elixir: z.number().min(0).default(0),
  ...
  story: z
    .object({
      ...
      heavySleeperHours: z.number().default(0),
    })
    .default({ seen: {}, merchantPurchases: 0, heavySleeperHours: 0 }),
  ...
  /**
   * Playlight exit-intent banners shown in normal play (0–5). When playTime reaches
   * `PLAYLIGHT_EXIT_INTENT_MILESTONES_MS[index]`, one show is allowed then index increments.
   */
  playlightExitIntentMilestoneIndex: z.number().int().min(0).max(5).default(0),
  ...
  lastFreeGoldClaim: z.number().default(0),
  ...
  g: z.enum(["m", "f"]).optional(), // Estimated gender from Google account display name
