        ...
        <div className="flex flex-wrap gap-2">
            ...
            <HoverCard key="bastion" openDelay={100} closeDelay={100}>
              ...
              <HoverCardContent className="w-auto p-2">
                <div className="text-xs whitespace-nowrap">
                  {getCostText('buildBastion', useGameStore.getState()).replace(/-/g, '').replace(/\d+/g, (match) => Math.floor(parseInt(match) * 0.5).toString())}
                </div>
              </HoverCardContent>
            </HoverCard>
          )}
          ...
          {watchtowerDamaged && buildings.watchtower > 0 && (
            <HoverCard key="watchtower" openDelay={100} closeDelay={100}>
              ...
              <HoverCardContent className="w-auto p-2">
                <div className="text-xs whitespace-nowrap">
                  {getCostText('buildWatchtower', useGameStore.getState()).replace(/-/g, '').replace(/\d+/g, (match) => Math.floor(parseInt(match) * 0.5).toString())}
                </div>
              </HoverCardContent>
            </HoverCard>
          )}
          ...
          {palisadesDamaged && buildings.palisades > 0 && (
            <HoverCard key="palisades" openDelay={100} closeDelay={100}>
              ...
              <HoverCardContent className="w-auto p-2">
                <div className="text-xs whitespace-nowrap">
                  {getCostText('buildPalisades', useGameStore.getState()).replace(/-/g, '').replace(/\d+/g, (match) => Math.floor(parseInt(match) * 0.5).toString())}
                </div>
              </HoverCardContent>
            </HoverCard>
          )}
