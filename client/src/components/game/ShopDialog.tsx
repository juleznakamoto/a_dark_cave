...
function isShopItemPurchaseDisabled(
  item: ShopItem,
  opts: {
    isUserSignedIn: boolean;
    lastFreeGoldClaim: number;
    purchasedItems: string[];
  },
): boolean {
  if (item.price === 0 && item.id !== "gold_100_free" && !opts.isUserSignedIn) {
    return true;
  }
  if (
    item.id === "gold_100_free" &&
    (Date.now() - (opts.lastFreeGoldClaim || 0)) / (1000 * 60 * 60) < 24
  ) {
    return true;
  }
  if (
    ...
    opts.purchasedItems.some((pid) => purchaseIdToItemId(pid) === item.id)
  ) {
    return true;
  }
  return false;
...
function getCheckoutPriceBreakdown(
  ...
  const catalogSaleDiscountCents =
    catalogSaleListCents != null ? catalogSaleListCents - catalogCents : 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        const result = await verifyPaymentWithRetry(paymentIntent.id, user.id);
...
function ShopDialogOpen({ isOpen, onClose, onOpen }: ShopDialogProps) {
  ...
  const handlePurchaseClick = async (itemId: string) => {
      ...
      try {
        ...
        if (itemId === "gold_100_free") {
          ...
          const freeGoldAmount =
            item.rewards.resources?.gold ??
            SHOP_ITEMS.gold_100_free.rewards.resources?.gold ??
            0;
          gameState.addLogEntry({
            id: `free-gold-claimed-${Date.now()}`,
            message: t("ui:shop.freeGoldClaimedLog", {
              amount: freeGoldAmount,
            }),
            timestamp: Date.now(),
            type: "system",
          });
    ...
    if (!item.canPurchaseMultipleTimes) {
      ...
      if (
        existingRows &&
        userOwnsShopItemFromPurchaseRows(itemId, existingRows)
      ) {
        gameState.addLogEntry({
                  ...
                  >
                    ...
                    >
                      {(selectedFilter === null
                        ...
                        .map((item) => (
                          <ShopItemGlyphParticleScope key={item.id} item={item}>
                            {({ hoverHandlers, portal, glyphOriginRef }) => {
                              const purchaseDisabled =
                                isShopItemPurchaseDisabled(item, {
                                  isUserSignedIn: !!gameState.isUserSignedIn,
                                  lastFreeGoldClaim:
                                ...
                                <>
                                  <Card
                                    ...
                                    onClick={(e) => {
                                      ...
                                      if (
                                        target.closest(
                                          "button, [data-tooltip-trigger-id]",
                                        )
                                      ) {
                                        return;
                                      }
                                    ...
                                    className={`border-neutral-500 flex flex-col relative transition-[outline-color] duration-400 hover:outline hover:outline-2 hover:outline-offset-[-2px] ${purchaseDisabled ? "" : "cursor-pointer"} ${item.category === "bundle"
                                      ...
                                      : item.id === "cruel_mode" &&
                                        ...
                                        ? "border border-red-600 outline-red-500"
                                        : "outline-neutral-400"
                                      }`}
                                  >
                                              ...
                                              {showTradersGratitudeInfo && (
                                                <TooltipWrapper
                                                  tooltip={
                                                    <div className="text-xs">
                                                      {t(
                                                        "ui:shop.tradersGratitudeDiscount",
                                                        {
                                                          percent:
                                                            TRADERS_GRATITUDE_DISCOUNT_PERCENT,
                                                        },
                                              ...
                                              {showJourneyCompleteInfo && (
                                                <TooltipWrapper
                                                  tooltip={
                                                    <div className="text-xs">
                                                      {t(
                                                        "ui:shop.journeyCompleteDiscount",
                                                      )}
                                                    </div>
                                                  }
                                              ...
                                              ? (Date.now() -
                                                ...
                                                ? (() => {
                                                  ...
                                                  return hoursRemaining === 1
                                                    ? t(
                                                      "ui:shop.availableInOneHour",
                                                    )
                                                    : t(
                                                      "ui:shop.availableInHours",
                                                      {
                                                        hours: hoursRemaining,
                                                      },
                                                    );
