...
export default function EventDialog({
  ...
  // Generate merchant choices when merchant event opens (now handled by event definition)
  useEffect(() => {
    if (event?.id.includes("merchant")) {
      // Merchant choices are now generated by the event definition itself.
      // If dynamic generation is still needed, it should be handled within the event definition logic.
      // For now, we ensure eventChoices are set from event.choices.
      setEventChoices(event?.choices || []);
    }
  }, [event?.id, event?.choices]);
