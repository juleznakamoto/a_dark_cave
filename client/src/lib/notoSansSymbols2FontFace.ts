/**
 * @font-face blocks for Noto Sans Symbols 2 (same payload Chrome receives from the CSS API).
 * Served via a same-origin `<style>` instead of `<link href="https://fonts.googleapis.com/...">`
 * so scripts that iterate `document.styleSheets[].cssRules` (Playlight) do not hit cross-origin
 * sheets and throw SecurityError.
 *
 * Source URL (Chrome UA): https://fonts.googleapis.com/css2?family=Noto+Sans+Symbols+2&display=swap
 */
...
function ensurePreconnect(href: string, crossOrigin?: string) {
  const sel = `link[data-noto-preconnect="${href}"]`;
  if (document.querySelector(sel)) return;
  const link = document.createElement("link");
  link.rel = "preconnect";
  link.href = href;
  if (crossOrigin) link.crossOrigin = crossOrigin;
  link.setAttribute("data-noto-preconnect", href);
