import { isHashRoutedPortalBuild } from "@/lib/edition";

/**
 * Resolve an in-app path (`/end-screen`, `/?game=true`) against the current
 * document URL. CrazyGames and itch host the folder in a subdirectory and
 * route via the hash, so a raw `location.href = "/end-screen"` would leave
 * the game.
 */
