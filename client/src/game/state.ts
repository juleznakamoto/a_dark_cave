...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  setCooldown: (action: string, duration: number) => {
    set((state) => {
      ...
      const newState = {
        cooldowns: { ...state.cooldowns, [action]: endTime }, // Store end timestamp
        cooldownDurations: { ...state.cooldownDurations, [action]: duration }, // Store original duration
      };
      return newState;
    });
  },
  ...
  tickCooldowns: () => {
    set((state) => {
      ...
      for (const key in newCooldowns) {
        ...
        // If the end timestamp has passed, remove the cooldown
        if (endTime <= now) {
