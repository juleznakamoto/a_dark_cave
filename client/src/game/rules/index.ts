...
// Add Grand Blacksmith building
gameActions.buildGrandBlacksmith = {
  cost: {
    1: {
      resources: {
        stone: 10000,
  ...
  effects: {
    story: {
      seen: {
        grandBlacksmithBuilt: true,
      },
    },
    logMessages: ["Now possible to forge weapons of the strongest materials."],
  },
  building: true,
  show_when: {
    "buildings.blacksmith >= 1": true, // Requires a Blacksmith to be built
    "story.events.found_frostglass_in_hill_grave": true, // Event trigger
  },
};

// Add Frostglass Sword forging action
gameActions.forgeFrostglassSword = {
  cost: {
    resources: {
      frostglass: 50,
    },
  ...
  effects: {
    ...
    logMessages: ["You forged the Frostglass Sword!"],
  },
  show_when: {
    "buildings.grandBlacksmith >= 1": true, // Requires Grand Blacksmith
    "resources.frostglass >= 50": true, // Requires Frostglass
  },
  handle: (state: GameState) => {
    // Handle the logic for forging the Frostglass Sword
    // This would typically involve applying the costs and effects defined above.
    // For now, we'll assume applyActionEffects handles this.
    return applyActionEffects("forgeFrostglassSword", state);
  },
};

// Add the event for finding the frostglass
gameActions.foundFrostglassInHillGrave = {
  effects: {
    story: {
      seen: {
        found_frostglass_in_hill_grave: true,
      },
    },
    logMessages: [
      "After you found the frostglass in the hill grave, the wizard says that you need to upgrade your Blacksmith to be able to forge the frostglass sword.",
    ],
  },
  show_when: {
    "story.events.hill_grave_explored": true, // Assuming this event is triggered before
    "story.events.found_frostglass_in_hill_grave": false, // Only show once
  },
};