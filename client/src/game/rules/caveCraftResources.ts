...
export function handleCraftVoidBomb(state: GameState, result: ActionResult): ActionResult {
  ...
  if (!state.story.seen.hasVoidBomb) {
    result.logEntries!.push({
      ...
      message: "The void bomb materializes with a reality-warping shimmer. Its core pulses with the darkness between stars, a weapon of devastating power that should not exist in this realm.",
      timestamp: Date.now(),
      type: 'system',
    });
  }
