...
// Helper to get resource gains tooltip for actions
export const getActionGainsTooltip = (actionId: string, state: GameState): string | null => {
  ...
  // Get effects (handle both function and object)
  const effects = typeof action.effects === 'function' ? action.effects(state) : action.effects;

  // Process each effect
  Object.entries(effects).forEach(([path, effect]: [string, any]) => {
    if (!path.startsWith('resources.')) return;

    const resource = path.split('.')[1];
    const resourceName = resource.split('_').map((word: string) => 
    ...
    // Handle fixed amounts
      ...
      if (amount > 0) {
        ...
        costs.push(`${amount} ${resourceName}`);
      }
    }
  });

  // For mine actions, show gains then costs
  if (actionId.includes('mine') || actionId.includes('Mine')) {
    const combined = [...gains, ...costs];
    return combined.length > 0 ? combined.join(', ') : null;
  }

  // For other actions (gather, hunt), only show gains
