...
export function startVersionCheck(onNewVersionDetected: () => void | Promise<void>) {
  // Always keep the latest callback so re-mounts re-wire correctly.
  ...
  // Check every 5 minutes while the app stays open...
  const CHECK_INTERVAL = 5 * 60 * 1000;

  // ...plus shortly after load, and whenever the app regains focus/visibility
  // (the key mobile case: reopening the installed app after a deploy).
  setTimeout(() => {
    void checkVersion();
  }, 3000);
