...
describe("playlightExitIntent", () => {
  it("returns the milestone at the current index when playTime qualifies", () => {
    expect(getActivePlaylightExitMilestone(H, 0)).toBe(
      PLAYLIGHT_EXIT_INTENT_MILESTONES_MS[0],
    );
    expect(getActivePlaylightExitMilestone(2 * H, 1)).toBe(
      PLAYLIGHT_EXIT_INTENT_MILESTONES_MS[1],
    );
  });

  it("returns null when playTime is below the next milestone", () => {
    expect(getActivePlaylightExitMilestone(H - 1, 0)).toBeNull();
  });
  ...
  it("floors migration from playTime", () => {
    ...
    expect(playlightExitIntentMilestoneFloorFromPlayTime(H)).toBe(1);
    expect(playlightExitIntentMilestoneFloorFromPlayTime(2 * H)).toBe(2);
    expect(playlightExitIntentMilestoneFloorFromPlayTime(10 * H)).toBe(4);
  });
});
