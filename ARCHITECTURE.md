...
- **`index.ts`**: i18next bootstrap; English `ui/shell` + `ui/seo` are seeded at init so React
  ...
  UI namespace is assembled from shards under `locales/{lang}/ui/` (`publicPages.json` is FAQ/About body copy). Direct visits to `/faq`, `/about`, `/press`, and legal routes skip i18n and the game SPA (`staticDocumentPath.ts`). Press kit body copy stays English in `shared/pressKit.ts`.
