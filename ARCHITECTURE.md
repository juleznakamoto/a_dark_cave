...
| `pages/` | Route-level components (lazy-loaded) | `start-screen-page.tsx`, `game.tsx`, `end-screen.tsx`, `reset-password.tsx`, `withdrawal.tsx`, `not-found.tsx`, `admin/dashboard.tsx`, `starship-shader-demo.tsx` (dev-only `/dev/starship-shader` preview), `estate-bar-upgrade-demo.tsx` (dev-only `/dev/estate-bar-upgrade` — Estate tab segmented bar grow + spark animation) |
