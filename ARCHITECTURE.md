...
| `achievements/` | Achievement configs, charts, claim logic | `AchievementMiniRingChart.tsx` (sizeable ring donut), `achievementProgress.ts` (overall/per-category % complete), `configs/` (basic/building/item/action + DEV-only non-claimable `overall`) |
