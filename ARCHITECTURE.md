...
| `supabase/` | SQL migrations + edge function (`functions/save-game/`) for Postgres/RLS. Notable: `024` deep-merge saves, `025` permanent tools/weapons/books protection, `030` flagged full-document replace on V1 (`p_full_replace`; old clients keep deep-merge), `034` admin session intra-day stats RPC, `035` drop abandoned `game_state_v2` dual-write sidecar (historical `028`/`029`). |
