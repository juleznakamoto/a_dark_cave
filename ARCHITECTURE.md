...
| `components/ui/` | shadcn/ui design system + game visuals | `button.tsx`, `card.tsx`, `dialog`, `toast.tsx`, `bubbly-button.components.tsx` (inline/global click particle rendering; `BubblyButton`, shared portal content), `button-particle-portal.tsx` (shared in-panel action-button click bursts; `ButtonParticlePortalProvider` in `GameContainer`, consumed by `CooldownButton`), `bubbly-button.particles.ts` (craft/mine/explore burst presets), `mist-background.tsx`, `cloud-shader.tsx`, `starship-shader.tsx` (WebGL2 fullscreen starship fragment shader), `spooky-smoke-animation.tsx` (WebGL2 blood moon smoke overlay), `limelight-nav.tsx` |
