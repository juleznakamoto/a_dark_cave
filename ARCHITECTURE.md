...
save header or `forceGame` intent (returning players skip the start-screen chunk, unless a
title click set the session `preferStartScreen` flag).
...
unless `preferStartScreen` is set),
