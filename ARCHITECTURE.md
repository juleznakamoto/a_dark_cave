...
- **`startupIntent.ts` / `startupUrlCleanup.ts` / `startupCoordinator.ts` / `startupGameLoader.ts` /
  ...
  transfer any store prepared during auth/Steam checks or Light Fire, and run ordered gameplay
...
| `client/src/components/game/CrazyGamesMenuLinks.tsx` | Steam + Reddit / Instagram / YouTube / Email in the header menu (footer hides those on CrazyGames). |
